  ...
  return (
    <footer className="bg-primary-950 text-slate-300">
            ...
            <div className="mt-6 flex gap-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/5 text-slate-300 transition-colors hover:bg-accent-500 hover:text-white"
                >
        ...
        <div className="mt-12 grid gap-4 border-t border-white/10 pt-8 sm:grid-cols-3">
          {contactItems.map((item) => (
            <div key={item.value} className="flex items-start gap-3 text-sm">
              ...
              {item.href ? (
                <a href={item.href} className="text-slate-300 hover:text-accent-400">
                  {item.value}
                </a>
              ) : (
      ...
      <div className="border-t border-white/10">
        <div className="section-container flex flex-col items-center justify-between gap-3 py-5 text-sm text-slate-500 sm:flex-row">
          <p>
